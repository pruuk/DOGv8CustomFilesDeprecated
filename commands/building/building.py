...
class BiomeWormCmd(Command):
    """
    Resets the biomes for a room to the base default and applies all of the
    biomes to all of the rooms in the zone.
    """
    key='@biomeworm'
    locks = 'cmd:id(1)'
    help_category = 'Building'
    ...
    def func(self):
        ...
        room = self.caller.search(self.args.strip(), global_search=True)
        if room and utils.inherits_from(room, 'typeclasses.rooms.Room'):
            bworm = create_object('commands.building.building.BiomeWorm',
                                 key='bworm',
                                 location=room)
            biome_reset = len(bworm.has_mapped_room_ids)
            self.msg(f"{biome_reset} rooms have been updated with fresh biomes.")
            bworm.delete()
        else:
            self.msg("Searched room not found. provide a valid room for the worm to map")


class BiomeWorm(Object):
    """
    A worm that travels through a zone and sets the biomes of the rooms.
    Called by the 'biomeworm' command.
    """
    ...
    def explore_map(self, room):
        ...
        Explores the map via exits.
        """
        self.set_room_biomes(self.location)
        for exit in room.exits:
            if exit.name not in ("north", "east", "west", "south"):
                # we only map in the cardinal directions. Mapping up/down would be
                # an interesting learning project for someone who wanted to try it.
    ...
    def set_room_biomes(self, room):
        """
        Sets the X and Y Coordinate traits for the
         room to match CurX and CurY
        """
        if room == self.location:
            self.has_mapped[room] = [self.curX, self.curY]
            self.has_mapped_room_ids.append(room.id)
